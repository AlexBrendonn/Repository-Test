...
import com.saa.web.control.tillage.ServiceTypeControl;
...
@Path("/tillage/service_type")
public class ServiceTypeRouter {
    @Context
    ...
    public Response GET() {
        this.control = new ServiceTypeControl((AuthenticationToken) webRequest.getSession().getAttribute("authentication"));
        return control.list();
    }
    ...
    public Response PUT(String content){
        this.control = new ServiceTypeControl((AuthenticationToken) webRequest.getSession().getAttribute("authentication"));
        JSONObject body = new JSONSchema().valid(ESchema.SERVICE_TYPE, content);
