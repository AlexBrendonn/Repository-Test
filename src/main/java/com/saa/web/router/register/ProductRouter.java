...
import com.saa.web.control.register.ProductControl;
...
public class ProductRouter {
    ...
    public Response GET() {
        this.control = new ProductControl((AuthenticationToken) webRequest.getSession().getAttribute("authentication"));
        return control.list();
    }
    ...
    public Response PUT(String content){
        this.control = new ProductControl((AuthenticationToken) webRequest.getSession().getAttribute("authentication"));
        JSONObject body = new JSONSchema().valid(ESchema.PRODUCT, content);
